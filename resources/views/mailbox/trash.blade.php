...
<div class="sidebar">
    <nav>
        ...
        <ul class="nav nav-pills flex-column mb-auto">
            ...
            <li><a class="nav-link active margin-20" href="/trash">Corbeille</a></li>
...
<div class="content">
    <?php
    $inbox_emails = app('App\Http\Controllers\mailbox_controller')->get_email_with_category(Auth::user()->id, app('App\Http\Controllers\mailbox_controller')->get_category_id_by_name('trash'));
    if (empty($inbox_emails)) {
        echo '<div class="picture">';
        echo '<img class="picture center" src="http://127.0.0.1:8000/images/mail.png" alt="img">';
        echo '<h2 class="text-center">Aucun message dans la corbeil</h2>';
        echo '</div>';
    }
    ?>
        ...
        <div class="row no-margin">
            ...
            <div class="col">
                <form action="/add-to-starred" method="post">
                    ...
                    <button type="submit" class="btn btn-outline-primary">Récuperer</button>
                </form>
                <br>
                <form action="/add-to-trash" method="post">
                    @csrf
                    <input type="hidden" name="email_id" value="<?php echo $email->id; ?>">
                    <button type="submit" class="btn btn-outline-danger">Suppprimer</button>
                </form>
            </div>
        </div>
        <hr>
